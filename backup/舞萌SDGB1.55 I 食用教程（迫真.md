#一般程度的垃圾教程，写的比较乱，见谅,，部分资源依旧是dx2025的 后续有空会更新

> [!WARNING]
> 下载SDGB懒人包（山东广播） ：不出示下载链接,需要的可以等人评论区发（fa wo SDEZ1.66）**

如果你自己拼的包,建议先看我另一篇[文章](https://ymgjdsh.github.io/yinglaoqi/post/SDGB-de-jie-bao.html)
来了解基本的Package构造(不提供.app资源，因为这玩意其实挺好找的)
...
_Ps：可以从一些QQ BOT或者tgbot 里获得userid 但同时你的账号信息有概率泄漏_
以后就可以用这个UserID直接在刚刚那个悬浮窗下面登录 但是前提是30min前刷新过二维码(不然会关你15分钟小黑屋）


8.如何更新?（有压缩包的情况，若只有opt文件 请跳转到另一篇文章opt解包教程）
解压1.50 X更新包压缩包 你会看见例如“A071”的文件夹（不是就改成这样的格式比如A03x）打开看一下里面有没有套娃 如果没有  就把文件夹丢到SDGB2026\Package\Sinmai_Data\StreamingAssets 打开游戏就可以发现更新了 (积累更新包必须一起放入)

9.如何开启自动游玩AUTOPLAY 如何解锁全部收藏品/歌曲/搭档？？
**这里以大多数懒人包最常用的MOD为例，当然aquamai也差不多**
...
解压后 直接全选文件夹里的文件 复制到D:\SDGB2026\Package并全部替换
...
_目前只有bing引擎能搜到此文章，权重这么低吗?（2026.6.13）_
