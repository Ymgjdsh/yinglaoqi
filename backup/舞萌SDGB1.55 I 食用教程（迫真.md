...
**方案二（UserID登录）**：  
上机玩一把之后，上传成绩，然后去你的userdataout文件夹（自己找找在哪里）获得userId。  
...
解压1.50 X更新包压缩包，你会看见例如“A071”的文件夹（不是就改成这样的格式比如A03x），打开看一下里面有没有套娃，如果没有，就把文件夹丢到 `SDGB2026\Package\Sinmai_Data\StreamingAssets`，打开游戏就可以发现更新了（积累更新包必须一起放入）。
